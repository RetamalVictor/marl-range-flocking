...
class MultiAgentEnv(gym.Env):
    ...
    def reset(self):
        ...
        self.headings =  (0 - np.pi*2) * torch.rand(self.num_particles).cuda() + np.pi* 2
    ...
    def _computeReward(self):
        ...
        rewards = collision_penalty.reshape(-1, 1) + center_of_mass_reward.reshape( -1, 1) + angular_velocity_penalty.reshape(-1, 1) #+ desired_distance_reward.reshape(-1,1)#+ distance_regions.reshape(-1, 1) 
        return rewards
    ...
    def _updateState(self, action, dt, heading:bool = False):
        """
        ...
        if heading:
            ...
            self.headings += angular_velocity * dt**2
